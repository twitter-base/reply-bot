...
const // there are more const function variables to follow,
    ...
    removeWebhooks = async () => {const _port = await getNewPort();  return new Autohook({port: _port, }).removeWebhooks();},
    processLog = reason => {
    ...
    getSignature = ( user_screen_name, bot_screen_name, tweet_tag_or_id, signature ) => {
    ...
    appendLog = ( log_dir, module_name, log_name, jsonObject, optional_date_time ) => {
        const filename =
          trimPath(log_dir) + '/' +
          module_name +
          '.' +
    ...
    normalizeNewlines = text => text.replaceAll(/\n\n+/gu, '\n\n').replaceAll(/\n\n+/gu, '\n\n'),
    ...
    collapseExtraSingleSpaces = text => {
        ...
        let count = 0;
        if (typeof spaceMatches !== 'undefined' && spaceMatches && spaceMatches.length > 0) {
            spaceMatches.forEach(match => {
                output = output.replaceAll(match, match.charAt(0).replaceAll(/\S/gu, '') + match.replaceAll(/\s/gu, '') + match.charAt(match.length - 1).replaceAll(/\S/gu, ''));
                count += 1;
            });
        }
        if (typeof newlineMatches !== 'undefined' && newlineMatches && newlineMatches.length > 0) {
            newlineMatches.forEach(match => {
                const intermediate = match.split('\n\n');
                count += intermediate.length;
                output = output.replaceAll(match, match.charAt(0).replaceAll(/\S/gu, '')+ intermediate.map(value => value.replaceAll(/\s/gu, '')).join('\n\n') + match.charAt(match.length - 1).replaceAll(/\S/gu, ''));
            });
        }
        if (typeof tabMatches !== 'undefined' && tabMatches && tabMatches.length > 0) {
    ...
    removeEmoji = text =>  { const regex = emojiRegex();return collapseExtraSingleSpaces(normalizeSpaces(text.toString().replaceAll(regex, ' ').replaceAll(/\s\n\s/gu, ' \n'))); },
    ...
    isEmojiRidden = text => {
        ...
        if (_emojiCount > 3 && (_emojiCount > spaceCount * 0.5 || _emojiCount > whitespaceCount || _emojiCount > characterCount * 0.25 || firstCharactersEmojiCount > 3)) {
            return _emojiCount;
        } else {
            return false;
        }
    ...
    cleanText = text => {console.dir({text,});return normalizeCharacters(unlink(text)); },
    cleanEmoji = text => {
            ...
            urls.forEach(urlValue => {
                output = output
                    .replaceAll(
                        ...
                        (typeof skipUrls !== 'undefined' ? [skipUrls,].flat(Infinity) : []).includes(urlValue.expanded_url) ?
                            ' [\'Quote-Tweet\'] ' :
                                        ...
                                        async tweet => {
                                            ...
                                            if (
                                                ...
                                                appendLog(
                                                    log_dir,
                                                    bot_name,
                                                    key + '.tweetThreadReply.bugReport.prelim',
                                                    { value, event, }
                                                );
                                            }
                                            if (tweet.user.id_str === bot_user_id) {
                                                ...
                                                const in_reply_to_status_id =
                                                    ...
                                                    url_expanded_text = urlExpand((getProperty(
                                                        extended_tweet_reply_to,
                                                        'full_text',
                                                        extended_tweet_reply_to.text
                                                    ) || '').substring(
                                                        reply_to_display_text_range[0]), urls, media, quoted_status_permalink_expanded),
                                                ...
                                                if (
                                                    ...
                                                    appendLog(
                                                        log_dir,
                                                        bot_name,
                                                        key + '.tweetThreadReply.bugReport',
                                                        { status_reply_to, event, }
                                                    );
                                                }
                                                if (
                                                    user_tweet_text
                                                ...
                                                return {sub_type: 'other.etc',}; // eslint-disable-line no-unreachable
                                            }
                                        }
                                    ),
