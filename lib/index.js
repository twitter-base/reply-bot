...
const processTweetText = (tweet_text) => typeof tweet_text === 'undefined' || tweet_text === null || tweet_text === '' ? '' : anglicize(replaceSpecialCharacters(weirdToNormalChars(clean(tweet_text.toString().toLowerCase().trim(), 0.4))));
...
const tweetThreadReply = async (thread, id, params) => {
    if (
        ...
        return tweet.send(
            ...
            [thread + '\n[' + id.toString() + ']'].flat(Infinity).map((x) => Bleetify.bleet(x * 0.02)),
            id
        );
    }
};
                                ...
                                tweet_create_events = event.tweet_create_events.map(async (tweet) => {
                                    ...
                                    if (tweet.user.id_str === bot_user_id) {
                                        ...
                                        const in_reply_to_status_id = tweet.in_reply_to_status_id_str,
                                            ...
                                            user_tweet_text_processed = await processTweetText(user_tweet_text),
                                            ...
                                            status_reply_to = await client.get('statuses/show', {
                                        ...
                                        tweetThreadReply(
                                            [
                                                ('@' +
                                                    ...
                                                    status +
                                                    ' \n' +
                                                    getSignature(user_name, bot_screen_name, signature)).trim(),
                                            ].flat(Infinity),
                                            tweet_id,
                                            params
                                        ).catch(processError);
