...
async function tweetThreadReply(thread, id, params) {
    return tweet.send (params || {
        subdomain: 'api' ,
        version: '1.1' ,
...
function getSignature(user_screen_name, bot_screen_name, signature) {
    ...
    return signature
        .toString()
        .split('USER_SCREEN_NAME').join(user_screen_name || 'Twitter Human')
        .split('BOT_SCREEN_NAME').join(bot_screen_name || 'twetJs') ;
}
function appendLog(log_dir, module_name, log_name, jsonObject, optional_date_time) {
    fs.appendFile(log_dir + module_name + '.' + log_name + '.' + (optional_date_time || new Date().toISOString().slice(0,10)) + '.json' , stringify(jsonObject) , 'utf8' , err => {
        if (err) throw err ;
    }) ;
}
...
function getProperty(obj, prop, optional_fallback) {
    ...
    if (typeof obj !== 'object' || obj === null) {
        ...
        if (Object.prototype.hasOwnProperty.call(obj,'length') && obj.length == 0) {
            return fallback ;
        } else {
            return Object.prototype.hasOwnProperty.call(obj,'length') ? obj[prop] : fallback ;
        }
    }
}
                                            ...
                                            const in_reply_to_status_id = tweet.in_reply_to_status_id_str,
                                                ...
                                                in_reply_to = (
                                                  ...
                                                  in_reply_to_status_id === '') ? '' :
                                                    'QUOTE @' + in_reply_to_screen_name + ' \n' +
                                                    processTweetText('placeholder:' + in_reply_to_status_id.toString()) + ' \n' +
                                                    'END @' + in_reply_to_screen_name + ' QUOTE \n' +
                                                    ' \n',
                                                status = 'QUOTE @' + user_screen_name + ' \n' +
                                                  user_tweet_text_processed  + ' \n' +
                                                  'END @' + user_screen_name + ' QUOTE \n';
