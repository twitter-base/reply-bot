...
const // there are more const function variables to follow,
    ...
    getEmoji = text => { const regex = emojiRegex(); console.dir({text,regex,});return text.toString().match(regex); },
    removeEmoji = text =>  { const regex = emojiRegex(); console.dir({text,regex,});return text.toString().replace(regex, '').replace(/\s\n\s/gu, ' \n'); },
    ...
    isEmojiRidden = text => {
        ...
        if (_emojiCount > spaceCount * 0.5 || _emojiCount > whitespaceCount || _emojiCount > characterCount * 0.25 || firstCharactersEmojiCount > 3) {
            return _emojiCount;
        } else {
            return false;
        }
